...
function addGlobalNodeFields(nodes, composer, concreteEntities) {
    const globalNodes = nodes.filter((n) => n.isGlobalNode);
    const globalEntities = concreteEntities.map((e) => new ConcreteEntityAdapter_1.ConcreteEntityAdapter(e)).filter((e) => e.isGlobalNode());
    if (globalNodes.length === 0)
        return false;
    const fetchById = (id, context, info) => {
        const resolver = (0, global_node_1.globalNodeResolver)({ nodes: globalNodes, entities: globalEntities });
