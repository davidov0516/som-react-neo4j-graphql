...
function getUserDefinedDirectives(definitionNodes) {
    ...
    for (const definitionNode of definitionNodes.unionTypes) {
        const userDefinedUnionDirectives = definitionNode.directives?.filter((directive) => !(0, is_in_array_1.isInArray)(constants_1.INTERFACE_DIRECTIVES, directive.name.value)) ||
            [];
        const propagatedDirectives = definitionNode.directives?.filter((directive) => (0, is_in_array_1.isInArray)(constants_1.PROPAGATED_DIRECTIVES, directive.name.value)) ||
            [];
